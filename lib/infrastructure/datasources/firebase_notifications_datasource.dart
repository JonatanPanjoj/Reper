...
class FirebaseNotificationsDatasource extends NotificationDatasource {
  ...
  Future<ResponseStatus> createInvitationGroupNotification(
    ...
    try {
      ...
      final existenceNotificationRes = await validateGroupRequest(
  ...
  Future<ResponseStatus> validateGroupRequest(
      {required String receiverId, required String groupId}) async {
    try {
      final snapshot = await _database
          .collection('notifications')
          .where(
            Filter.and(
              Filter('group_id', isEqualTo: groupId),
              Filter('receiver_id', isEqualTo: receiverId),
              Filter("type", isEqualTo: NotificationType.group.value),
              Filter.or(
                Filter("status", isEqualTo: 'waiting'),
                Filter("status", isEqualTo: 'accepted'),
              ),
      ...
      return ResponseStatus(message: 'Sin notificaciones', hasError: false);
    } on FirebaseException catch (e) {
      return ResponseStatus(
          message: e.message ?? 'An exeption occurred', hasError: true);
    } catch (e) {
